...
class Serveur
  ...
  def send_up_request(sock, filepath = '../Downloads/TestVideoOriginal.mp4', outname = 'TestVideoCopie.mp4')
    ...
    request = forge_header(head_type, outname, filesize)
  ...
  def upload(sock, filepath, filesize)
    ...
    puts "Time elapsed: #{temps}. Transferred #{(filesize/1024)/1024} MB. Transfer per second: #{(((filesize / 1024)/ 1024)/ temps).round(2)} MB/s"
